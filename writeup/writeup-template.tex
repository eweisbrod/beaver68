...
%              ->  pdflatex twice more
%   Overleaf:  upload this file, references.bib, and your output/ folder.
%              Put output/ either beside this file or one level up --
%              both are searched. Overleaf's free tier does not sync with
%              GitHub, so you upload output/ by hand after each run.
%
% NOTE ON PATHS
%   Generated tables and figures are looked for in ../output/, then
%   output/, then this folder, so the repo layout and a flat Overleaf
%   project both work with no edits. Anything not found in one of those
%   three shows up as a labelled placeholder box rather than stopping the
%   compile.
%
%   If you changed OUTPUT_DIR in .env to something other than output/,
%   either rename the folder when you upload it or add your path to
%   \graphicspath below and to the two \IfFileExists chains.
% =============================================================================
...
% -----------------------------------------------------------------------------
% GENERATED RESULTS, AND WHAT HAPPENS WHEN THEY ARE MISSING
%
% This file ships before you have run anything, so output/ is empty and
% every table and figure below is absent. Pulling a missing file in
% directly aborts the compile with "File not found", which is a poor way
% to meet a template. So each result goes through \resulttable or
% \resultgraphic, which check whether the file is there and drop in a
% visible placeholder when it is not.
%
% The document therefore always compiles. An unfilled slot shows up as a
% framed box on the page saying what is missing, rather than as an error
% you have to decode. Run src/run-all.R and recompile and the real
% tables and figures replace the boxes.
%
% Second argument is the human name of the result. It is also what
% writeup/make-writeup-docx.R turns into the "paste this here"
% instruction in the Word version, so keep it descriptive.
%
% THREE LOCATIONS ARE SEARCHED, in this order:
%
%   ../output/   the repo layout, with this file in writeup/
%   output/      this file and an output/ folder side by side
%   ./           everything in one flat folder
%
% The second and third exist for Overleaf, where projects are commonly
% flat and there is no folder above the root for "../" to reach. Getting
% this wrong used to be loud, an outright "File not found"; now it is a
% placeholder box, so the box names all three paths it tried.
% -----------------------------------------------------------------------------
\newcommand{\missingresult}[2]{%
  \begin{center}%
  \setlength{\fboxsep}{10pt}%
  \fbox{\parbox{0.75\textwidth}{%
    \centering
    \textbf{[ #1 -- not found ]}\par\medskip
    {\small\itshape Looked for \texttt{#2} in \texttt{../output/},
     \texttt{output/}, and this folder. Either run \texttt{src/run-all.R}
     to generate it, or move it to one of those places.}%
  }}%
  \end{center}%
}

% \resulttable{file stem}{human name}
%
% NOTE: each generated .tex file is a COMPLETE, self-contained table --
% it brings its own \begin{table}, \centering, and \caption. So it is
% \input{} bare. Wrapping it in another \begin{table} produces the error
% "LaTeX Error: Not in outer par mode."
\newcommand{\resulttable}[2]{%
  \IfFileExists{../output/#1.tex}{\input{../output/#1.tex}}{%
  \IfFileExists{output/#1.tex}{\input{output/#1.tex}}{%
  \IfFileExists{#1.tex}{\input{#1.tex}}{%
  \missingresult{#2}{#1.tex}}}}%
}

% \resultgraphic{file stem}{human name}
%
% Goes INSIDE a figure environment, so that the \caption and \label stay
% in plain LaTeX where you can edit them normally.
\newcommand{\resultgraphic}[2]{%
  \IfFileExists{../output/#1.pdf}{\includegraphics[width=0.85\textwidth]{../output/#1.pdf}}{%
  \IfFileExists{output/#1.pdf}{\includegraphics[width=0.85\textwidth]{output/#1.pdf}}{%
  \IfFileExists{#1.pdf}{\includegraphics[width=0.85\textwidth]{#1.pdf}}{%
  \missingresult{#2}{#1.pdf}}}}%
}
...
% Same three locations the \IfFileExists chains search, so that a plain
% \includegraphics you add yourself resolves the same way they do.
...
\resulttable{sample-selection}{Table 1: Sample selection}
...
\resulttable{descriptives}{Table 2: Descriptive statistics}
...
\begin{figure}[H]
\centering
\resultgraphic{fig1-volume}{Figure 1: Trading volume}
...
\begin{figure}[H]
\centering
\resultgraphic{fig2-return-variability}{Figure 2: Return variability}
...
\resulttable{main-results-returns}{Table 3: Return variability}

\resulttable{main-results-turnover}{Table 4: Trading volume (turnover)}
...
\begin{figure}[H]
\centering
\resultgraphic{fig3-turnover-by-decade}{Figure 3: Turnover by decade}
\caption{Median share turnover around annual earnings announcements, by decade.}
\label{fig:turnover}
\end{figure}

\resulttable{by-decade}{Table 5: By decade}
...
\begin{figure}[H]
\centering
\resultgraphic{fig6-my-partition}{Figure 4: Your own partition}
\caption{Announcement-period reaction, partitioned by your own variable.}
\label{fig:mypartition}
\end{figure}

\resulttable{my-partition}{Table 6: Your own partition}
