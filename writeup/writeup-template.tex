...
\newcommand{\missingresult}[2]{%
  ...
  \fbox{\parbox{0.75\textwidth}{%
    \centering
    \textbf{[ #1 -- not generated yet ]}\par\medskip
    {\small\itshape Expected \texttt{#2}. Run \texttt{src/run-all.R},
     then recompile.}%
  }}%
  \end{center}%
}
...
\newcommand{\resulttable}[2]{%
  \IfFileExists{../output/#1.tex}%
    {\input{../output/#1.tex}}%
    {\missingresult{#2}{output/#1.tex}}%
}
...
\newcommand{\resultgraphic}[2]{%
  \IfFileExists{../output/#1.pdf}%
    {\includegraphics[width=0.85\textwidth]{../output/#1.pdf}}%
    {\missingresult{#2}{output/#1.pdf}}%
}
